...
void test()
{
	...
	//auto pos = find(s.begin(), s.end(), 6);//algorithm中的函数auto pos = s.find(3);
	//两个find有差异
	//auto pos = s.find(6);//set中的查找,时间复杂度为O(log2N),二分查找,建议使用这个
	//迭代器是暴力查找,时间复杂度为O(N)
	if (pos != s.end())
	{
		s.erase(pos);
...
void test1()
{
	...
	itup = myset.upper_bound(60);//<60,返回第1个大于value的迭代器
	cout << *itlow << endl;
	cout << *itup << endl;
	//查找是左闭右开
...
void test5()
{
	...
	dict.insert(make_pair("string", "字符串"));
	dict.insert(make_pair("string", "字符"));
